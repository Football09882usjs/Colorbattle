...
function endRound(winner){ inRound=false; if(winner===Players[0]) scores.p1++; else scores.p2++; roundNumber++; if(scores.p1>=TARGET_SCORE||scores.p2>=TARGET_SCORE){ showSplash=true; started=false; setTimeout(()=>{ scores={p1:0,p2:0}; roundNumber=1; },1200); return; } const winnerId=(winner===Players[0])? 'p1' : 'p2'; const loserId = winnerId==='p1' ? 'p2' : 'p1'; awaitingPicks=[winnerId, loserId]; pickState.active=false; setTimeout(()=> showNextPick(),420); }
